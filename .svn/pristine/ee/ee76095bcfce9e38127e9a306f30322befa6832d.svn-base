	     ...
	     $('#example').DataTable( {
	        "ajax":{
		        ...
		        "columns": [        
		            ...
		            { "data": "plannumber" },
		            { "data": "number" },
		            { "data": function(obj){return "<span><center><a onclick=\"open_layer('记录','admindatarecordlist?plan_id="+obj.plan_id+"&flag=2','900','450')\">"+obj.makingplansnumber+"</a></center></span>"}},
		            { "data": "thistimeplansnumber" },
		            { "data": "lastnumber" },
		            { "data": function(obj){
		            	if(obj.czflag==1){
		            		return "<span><center><a onclick=\"open_layer('录入','adminplanadd?plan_id="+obj.plan_id+"&flag=2','900','450')\">录入</a></center></span>"
		            	}else{
...
function Excelbutton(){
	$.ajax({
	    ...
	    success: function(data) {
		    if(data.code==100){
		    	/* window.location.href="http://test.jikewangluo.cn/upload/制定计划-军区.xls"; */
		    	window.location.href="http://localhost:8081/depot/upload/制定计划-军区.xls";
		    }else{	
