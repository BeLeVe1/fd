...
<body style="width: 80%;padding-left:10%;">
	<form  method="post" class="form form-horizontal" id="forms"  enctype="multipart/form-data">
		...
		<div class="row cl" style="padding-top:2%;">
			...
			<div class="formControls col-xs-7 col-sm-9">
				<input type="password" class="input-text" value="" placeholder="" id="newpassword2" name="newpassword2">
			</div>
		</div>
		<div class="row cl" style="padding-top:2%;display:none;">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red"></span>admin_id：</label>
			<div class="formControls col-xs-7 col-sm-9">
...
function sub(){
    ...
    if(forms.newpassword.value==""){
    	layer.msg('新密码不能为空');
    	return false;
    }
    if(forms.newpassword2.value==""){
    	layer.msg('确认新密码不能为空');
    	return false;
    }
    if(forms.newpassword.value!=forms.newpassword2.value){
    	layer.msg('两次输入的密码不相同');
    	return false;
    }
    $.ajax( {  
        type : 'POST',  
        url : 'adminpasswordajax',  
        data : params,  
