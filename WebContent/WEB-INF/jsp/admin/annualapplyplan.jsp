...
<body>
	<div id="container">
		<div class="text-c" style="margin-top:1%;">
			<input type="text" name="jqcode" id="jqcode" placeholder="记账人" style="width:100px" class="input-text">
			<input type="text" name="bdcode" id="bdcode" placeholder="业务类型" style="width:100px" class="input-text">
			<input type="text" name="qccode" id="qccode" placeholder="货位" style="width:100px" class="input-text">
			<input type="text" name="qcname" id="qcname" placeholder="入库日期" style="width:100px" class="input-text">
		...
		<div class="text-c" style="margin-top:1%;">
			<button onclick="Excelbutton();" class="btn btn-success" style="background-color: #337AB7;" type="submit"></i><i class="Hui-iconfont">&#xe644;</i> 添加入库记录</button>
...
<!-- 						<th style="text-align: center;width: 1%"><input type="checkbox" onclick="ckAll();" id="allChecks" name="" value="" style="zoom:150%;"></th> -->
						<th style="text-align: center;width: 1%">记账人</th>
						<th style="text-align: center;width: 1%">业务类型</th>
						<th style="text-align: center;width: 1%">仓库</th>
						<th style="text-align: center;width: 1%">货位</th>
						<th style="text-align: center;width: 1%">入库日期</th>
...
<!-- 						<th style="text-align: center;width: 1%">单装用数</th> -->
						<th style="text-align: center;width: 1%">入库单号</th>
						<th style="text-align: center;width: 1%">入库类别</th>
						<th style="text-align: center;width: 1%">数量</th>
						<th style="text-align: center;width: 1%">本币无税金额</th>						
						<th style="text-align: center;width: 1%">结算日期</th>
						<th style="text-align: center;width: 1%">累计结算金额</th>						
						<th style="text-align: center;width: 1%">税率</th>			
...
// 		            { "data": "dzys" },
		            ...
		            { "data": function(obj){
		            	if(obj.czflag==1){
		            		return "<span><center><a onclick=\"open_layer('修改','adminapplyplanedit?sharedpart_id="+obj.dataprocess_id+"&jqcode="+obj.jqcode+"&bdcode="+obj.bdcode+"&qccode="+obj.qccode+"&qcname="+obj.qcname+"&plannumber="+obj.plannumber+"&realnumber="+obj.realnumber+"&number="+obj.number+"&lastnumber="+obj.lastnumber+"&comments="+obj.comments+"','900','450')\">修改</a>&nbsp;<a>删除</a></center></span>"
		            	}else{
	...
	$.ajax({
	    ...
	    success: function(data) {
		    if(data.code==100){
		    	...
		    	window.location.href="http://localhost:8081/depot/upload/年度申请计划-部队.xls";
		    }else{	
