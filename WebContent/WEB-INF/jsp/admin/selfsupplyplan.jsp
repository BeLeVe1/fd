...
<!-- 						<th style="text-align: center;width: 1%">单装用数</th> -->
						...
						<th style="text-align: center;width: 1%">计算需求数量</th>
						<th style="text-align: center;width: 1%">期初库存</th>
						<th style="text-align: center;width: 1%">当前库存</th>
						<th style="text-align: center;width: 1%">计划自筹数量</th>
...
// 		sort="number";
// 	}
// 	if(document.getElementById("realnumber").checked==true){
// 		sort="realnumber";
// 	}
...
// 		            { "data": "dzys" },
		            ...
		            { "data": function(obj){
		            	if(obj.czflag==1){
		            		return "<span><center><a onclick=\"open_layer('修改','adminselfsupplyplanedit?sharedpart_id="+obj.dataprocess_id+"&jqcode="+obj.jqcode+"&bdcode="+obj.bdcode+"&qccode="+obj.qccode+"&qcname="+obj.qcname+"&plannumber="+obj.plannumber+"&realnumber="+obj.realnumber+"&number="+obj.number+"&lastnumber="+obj.lastnumber+"&comments="+obj.comments+"','900','450')\">修改</a></center></span>"
		            	}else{
...
// function checkbtn3(){
// 	if(document.getElementById("number").checked==true){
// 		document.getElementById("realnumber").checked=false;
// 		selectbutton();
// 	}else{
// 		selectbutton();
// 	}
...
function Excelbutton(){
	$.ajax({
	    type: "POST",
	    url:'adminplanjqExcel',
	    success: function(data) {
		    if(data.code==100){
		    	/* window.location.href="http://test.jikewangluo.cn/upload/制定计划-部队.xls"; */
		    	window.location.href="http://localhost:8081/depot/upload/器材自筹计划-部队.xls";
		    }else{	
